...
# Mac users: uncomment the lines below
# import matplotlib
...
def euclidean_distance(point1, point2):
    ...
    """
    distance = 0
    for num in range(len(point1)):
        distance += ((point1[num] - point2[num]) ** 2)
...
def get_closest_centroid(point, centroids_dict):
    ...
    Example:
        ...
        Output:
            centroid1
    """
    min_dist = -1   # cuz dis cant be 0
    centroid = ""
    for (key, value) in centroids_dict.items():
        distance = euclidean_distance(point, value)
...
def update_assignment(list_of_points, centroids_dict):
    ...
    for point in list_of_points:
        ...
        ret_dict[closest_centroid].append(point)
    return ret_dict
...
def mean_of_points(list_of_points):
    ...
    mean = []
    for item in list_of_points[0]:
        mean.append(0)

    for point in list_of_points:
        for point_index in range(len(point)):
            mean[point_index] += point[point_index]
    for index in range(len(mean)):
        mean[index] = mean[index] / len(list_of_points)
    return mean
...
def update_centroids(assignment_dict):
    ...
    for (key, value) in assignment_dict.items():
        ret_dict[key] = mean_of_points(value)
    return ret_dict
