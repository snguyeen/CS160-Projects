...
def read_grid(file_path):
    ...
    for line_of_text in myfile:
        row = []
        for pixel in line_of_text.split(','):
            row.append(int(pixel))
        grid.append(row)
    myfile.close()
    return grid
...
def get_pixel_at(pixel_grid, i, j):
    ...
    if j < 0 or i < 0:
        return 0
    elif j >= len(pixel_grid[0]) or i >= len(pixel_grid):
        return 0
    else:
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    for x in range(-1, 2):
        for y in range(-1, 2):
            row = x + i
            column = y + j
            pixel_sum += get_pixel_at(pixel_grid, row, column)
...
def blur(pixel_grid):
    ...
    for x in range(len(pixel_grid)):
        row_array = []
        for y in range(len(pixel_grid[0])):
            row_array.append(average_of_surrounding(pixel_grid, x, y))
        blurred_grid.append(row_array)
...
blurred = blur(input_grid)

# Step E: Write the result to both output files
write_image(out_image_filename, blurred)
write_grid(out_grid_filename, blurred)
